...
function timKQ(){
  ...
  {
    document.getElementById("kq").innerHTML=`<h2 class="mt-5 text-danger text-center">Bạn đã đậu. Điểm tổng của bạn là: ${diemTong} </h2>`
  }
  else if(diemTong<diemChuanEL&&diemMonMot>0&&diemMonHai>0&&diemMonBa>0){
  document.getElementById("kq").innerHTML=`<h2 class="mt-5 text-danger text-center">Bạn đã rớt. Điểm tổng của bạn là: ${diemTong} </h2>`}
  else{
    document.getElementById("kq").innerHTML=`<h2 class="mt-5 text-danger text-center">Bạn đã rớt. Do có môn có điểm bằng 0 </h2>`
...
function Tinhtien(){
    ...
    if(0<SoKWEL&& SoKWEL<=50){
        ...
        document.getElementById("tt").innerHTML=`<h2 class="mt-5 text-danger text-center">Họ tên là: ${HotenEL} , số tiền điện phải trả là ${Tinhtientra} VNĐ  </h2>`
    }
